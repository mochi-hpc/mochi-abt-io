...
static void abt_bench(int           buffer_per_thread,
                      ...
                      double*       seconds);
static void abt_bench_nb(int           buffer_per_thread,
                         unsigned int  concurrency,
                         size_t        size,
                         double        duration,
                         const char*   filename,
...
static void  pthread_bench(int           buffer_per_thread,
                           unsigned int  concurrency,
                           size_t        size,
                           double        duration,
                           const char*   filename,
...
int main(int argc, char** argv)
{
    ...
    if (argc != 6) {
        fprintf(stderr,
                "Usage: concurrent-write-bench <write_size> <concurrency> "
                "<duration> <file> <buffer_per_thread (0|1)>\n");
        return (-1);
    }
    ...
    abt_bench(buffer_per_thread, concurrency, size, duration, argv[4],
              &abt_ops_done, &abt_seconds);
    printf("# ...ABT benchmark done.\n");

    printf("# Running ABT (nonblocking) benchmark...\n");
    abt_bench_nb(buffer_per_thread, concurrency, size, duration, argv[4],
                 &abt_nb_ops_done, &abt_nb_seconds);
    ...
    pthread_bench(buffer_per_thread, concurrency, size, duration, argv[4],
                  &pthread_ops_done, &pthread_seconds);
    printf("# ...pthread benchmark done.\n");

    /* output */
    printf("#<type>\t<concurrency>\t<write_size>\t<ops>\t<seconds>\t<MiB/s>\n");
    printf("abt\t%u\t%zu\t%u\t%f\t%f\n", concurrency, size, abt_ops_done,
           abt_seconds,
           ((((double)size * (double)abt_ops_done)) / abt_seconds)
               / (1024.0 * 1024.0));
    printf("abt_nb\t%u\t%zu\t%u\t%f\t%f\n", concurrency, size, abt_nb_ops_done,
           abt_nb_seconds,
           ((((double)size * (double)abt_nb_ops_done)) / abt_nb_seconds)
               / (1024.0 * 1024.0));
    printf("pthread\t%u\t%zu\t%u\t%f\t%f\n", concurrency, size,
           pthread_ops_done, pthread_seconds,
           ((((double)size * (double)pthread_ops_done)) / pthread_seconds)
               / (1024.0 * 1024.0));
                      ...
                      double*       seconds)
{
    ...
    *ops_done = next_offset / size;
    ...
    if (buffer_per_thread) {
        for (i = 0; i < concurrency; i++) free(args[i].buffer);
    } else
        free(buffer);
                         ...
                         double*       seconds)
{
    ...
    *ops_done = next_offset / size;
    ...
    for (i = 0; i < num_buffers; i++) free(buffers[i]);
    free(buffers);
...
static void write_abt_bench(void* _arg)
{
    ...
    while ((wtime() - arg->start_time) < arg->duration) {
        ...
        ret = abt_io_pwrite(arg->aid, arg->fd, arg->buffer, arg->size,
                            my_offset);
        assert(ret == arg->size);
    }
